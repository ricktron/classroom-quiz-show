...
function running(deadlineOffsetMs = 30_000, durationMs = 30_000): PublicResponseState {
  return {
    ...
    buzz: NO_BUZZ, boardOutcome: NO_OUTCOME,
  }
}
...
describe('what the class can read', () => {
  ...
  it('states paused, expired and stopped in words', () => {
    ...
    const { rerender } = render(
      <ResponseTimerDisplay
        response={{ armed: false, timer: { status: 'paused', durationMs: 30_000, remainingMs: 12_000 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME }}
        clock={clock}
      />,
    )
    ...
    rerender(
      <ResponseTimerDisplay
        response={{ armed: false, timer: { status: 'expired', durationMs: 30_000 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME }}
        clock={clock}
      />,
    )
  ...
  it('shows an armed clue with no timer, and no clock', () => {
    render(
      <ResponseTimerDisplay
        response={{ armed: true, timer: { status: 'idle' }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME }}
        clock={createManualClock(AT)}
      />,
    )
    expect(screen.getByTestId('rtd-armed')).toHaveTextContent('Buzzers armed')
...
describe('accessibility', () => {
  ...
  it('gives paused, expired, stopped and armed distinct accessible text', () => {
    ...
    const { rerender } = render(
      <ResponseTimerDisplay
        response={{ armed: true, timer: { status: 'paused', durationMs: 30_000, remainingMs: 5_000 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME }}
        clock={clock}
      />,
    )
    ...
    rerender(
      <ResponseTimerDisplay
        response={{ armed: false, timer: { status: 'interrupted', durationMs: 30_000, remainingMs: 5_000 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME }}
        clock={clock}
      />,
    )
...
describe('privacy', () => {
  it('renders none of the permanent host-only labels, at any status', () => {
    ...
    const states: PublicResponseState[] = [
      running(),
      { armed: true, timer: { status: 'idle' }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME },
      { armed: false, timer: { status: 'paused', durationMs: 1_000, remainingMs: 500 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME },
      { armed: false, timer: { status: 'expired', durationMs: 1_000 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME },
      { armed: false, timer: { status: 'interrupted', durationMs: 1_000, remainingMs: 500 }, buzz: NO_BUZZ, boardOutcome: NO_OUTCOME },
    ]
